...
def first_odd_or_even(numbers):
    ...
    if len(num_even) != 0 and len(num_odd) > len(num_even):
        ret = num_even[0]
    elif len(num_odd) != 0 and len(num_odd) < len(num_even):
        ret = num_odd[0]
...
def to_pilot_alpha(word):
    ...
    pilot_alpha_list = []
    for i in word.upper():
        re_find = re.compile(i+r'\w+')
        pilot_alpha_list.extend(list(filter(re_find.match, pilot_alpha)))
